...
.sq-background(:class="'sq-background--' + color")
  ...
  Rotator(
    ...
    :stop="stopStatus"
    @imdone="doSwitch"
    @outting="startShow = true"
  )
  Rotator(
    :deg="-rotation + 45"
    :reverse="true"
    shift="0,-15px,0"
    :scale="1.08 - 0.08 * scale"
    :stop="stopStatus"
  )
  ...
  transition(name="fade")
    span.sq-loading(
      ref="loader"
      v-if="!startShow"
      :disabled="preloadComplete"
      @click="goNext"
      :class="{ loading: !preloadComplete }"
    ) {{ text }}
...
export default {
  ...
  async mounted() {
    ...
    this.preloadImages = (await Promise.all(imagePreloaderPromise)).map(item => item.default);
